      ...
      <div className="max-w-4xl mx-auto w-full">
        ...
        <div className="grid grid-cols-2  sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-4">
          {skills.map((skill) => (
            <Card
              key={skill.name}
              className="group md:hover:shadow-2xl md:hover:shadow-primary/40 transition-all duration-300 md:hover:-translate-y-1 border-slate-300 bg-white/15 fade-in-delay-1"
            >
              <Link
                to={skill.url || ""}
                target="_blank"
